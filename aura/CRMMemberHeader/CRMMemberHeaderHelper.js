({
	...
	copyAttributesFromContact: function(component, event, helper) {
		...
		if (contact) {
			...
			// In the future, this should only be required if estatements are enabled.  For now, if the
			// email address is found, it can't be removed, only replaced.
			component.set('v.emailRequired', contact.Email !== '' && contact.Email !== null);
		}
	},
